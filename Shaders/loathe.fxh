...
#define linearstep(_min, _max, _x)                                             \
  (saturate(((_x) - (_min)) * rcp((_max) - (_min))))
#define where(_a, _b, _cond) ((_cond) ? (_a) : (_b))
#define UI_MESSAGE(_name, _text)                                               \
  uniform int _m(message, _name) < __UNIFORM_RADIO_INT1 ui_label = " ";        \
  ui_text = _n _text;                                                          \
  > = NULL
